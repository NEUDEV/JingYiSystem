...
<script type="text/javascript">
	$(document).ready(function getJSONData(pn) {
		$.ajax({
			...
			success : function(data) {
				...
				$.each(jsonObj, function(i, item) {
					jQuery("#courseList").append(
							"<tr><td>"+ item.uid + "</td>"+"<td>"+ item.stuid + "</td>"
							+"<td>"+ item.name + "</td>"+"<td>"+ item.phone + "</td>"
							+"<td>"+ item.qq + "</td>"+"<td>"+ item.weixin + "</td>"
							+"<td>"+ item.sfrom + "</td>"+"<td>"+ item.sign + "</td>"
							+"<td>"+ item.intime + "</td>"+"<td>"+ item.mark + "</td></tr>");
				});
			},
			error : function(text) {
			}
